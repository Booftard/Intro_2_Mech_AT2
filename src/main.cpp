...
String getLEDName(SystemState s) {
  ...
  void sendHeartbeat() {
    if (wifiState == WIFI_CONNECTED && client.connect(boardB_IP, boardB_Port)) {
        ...
        if (currentState == UNLOCKED) {
            heartbeatMessage += "UNLOCKED|READY";
        } else if (currentState == RED_LOCKED || currentState == RED_BLINK) {
            heartbeatMessage += "LOCKED|ACTIVE";
        } else {
            heartbeatMessage += "TRANSITION|ACTIVE";
        }
  ...
  void checkForIncomingMessages() {
    ...
    if (currentMillis - lastMessageCheckTime >= messageCheckInterval) {
        ...
        if (client) {
            ...
            while (client.connected() && client.available() && (millis() - startTime < 10)) {
  ...
  void connectToWiFi() {
    ...
    switch(wifiState) {
        ...
        case WIFI_CONNECTING:
            ...
            if (WiFi.status() == WL_CONNECTED) {
                ...
                systemReady = true;
                server.begin();  // Start server only after WiFi is connected
                if (authenticated) {
    ...
    switch(wifiState) {
        case WIFI_DISCONNECTED:
        case WIFI_FAILED:
            setSolidRGB(LOW, LOW, HIGH);  // Blue for disconnected/failed
            break;
        case WIFI_CONNECTING:
            // Blinking blue for connecting
